...
def print_model_parm_flops(model, input, detail=False):
    ...
    if detail:
        print('  + Conv FLOPs: %.6fG' % (sum(list_conv) / 1e9))
        print('  + Linear FLOPs: %.6fG' % (sum(list_linear) / 1e9))
        print('  + Batch Norm FLOPs: %.6fG' % (sum(list_bn) / 1e9))
        print('  + Relu FLOPs: %.6fG' % (sum(list_relu) / 1e9))
        print('  + Pooling FLOPs: %.6fG' % (sum(list_pooling) / 1e9))
