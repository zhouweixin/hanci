...
class NARRE_FA(nn.Module):
    """
    CNN 提取评论用户偏好和项目特征 + 特征注意力
    """
    ...
    def __init__(self, train_dset, word_embedding_size=300, id_embedding_size=32,
        ...
        self.user_feature_att = WordEmbedding(train_dset.user_num, train_dset.user_review_num, dropout=0.)
        self.item_feature_att = WordEmbedding(train_dset.item_num, train_dset.item_review_num, dropout=0.)
    ...
    def forward(self, user_id, item_id, user_reviews, item_reviews, user_rids, item_rids, rating, norm_lambda=0.001, save_att=False):
        ...
        # 3.1 feature attention
        user_fa = self.user_feature_att(user_id).transpose(1, 2) # [batch, user_review_num, 1]
        item_fa = self.item_feature_att(item_id).transpose(1, 2) # [batch, item_review_num, 1]

        # 3.2 multiply
        user_reviews_emb = user_fa * user_reviews_emb
        item_reviews_emb = item_fa * item_reviews_emb
