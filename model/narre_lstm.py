...
from model.language import WordEmbedding, ReviewCNN, ReviewLSTM
from model.attention import Attention
from util import utils


class NARRE_LSTM(nn.Module):
    """
    LSTM 提取评论用户偏好和项目特征
    """

    def __init__(self, train_dset, word_embedding_size=300, id_embedding_size=32,
                 hidden_size=100, num_layers=1, bidirectional=False, attention_size=32, num_latent=32, dropout=0.5):
        super(NARRE_LSTM, self).__init__()
        ...
        self.user_review_emb = ReviewLSTM(embedding_size=word_embedding_size, dropout=dropout,
                                          hidden_size=hidden_size, num_layers=num_layers, bidirectional=bidirectional)
        self.item_review_emb = ReviewLSTM(embedding_size=word_embedding_size, dropout=dropout,
                                          hidden_size=hidden_size, num_layers=num_layers, bidirectional=bidirectional)
    ...
    def forward(self, user_id, item_id, user_reviews, item_reviews, user_rids, item_rids, rating, norm_lambda=0.001, save_att=False):
        ...
        # 2.review_embedding
        user_reviews_emb = self.user_review_emb(user_word_emb)  # [batch, review_num, review_hidden_size]
        item_reviews_emb = self.item_review_emb(item_word_emb)  # [batch, review_num, review_hidden_size]
