...
@author: zhouweixin
@note: 改进版4：用CNN-LSTM + self-attention提取特征
"""

import torch
from torch import nn
from model.language import WordEmbedding, ReviewCNNLSTMSA
from model.attention import Attention
from util import utils


class NARRE_CNN_LSTM_SA(nn.Module):
    """
    CNN-LSTM 提取评论用户偏好和项目特征
    """

    def __init__(self, train_dset, word_embedding_size=300, id_embedding_size=32,
                 filter_sizes=[3], num_filters=100, hidden_size=100, num_layers=1, bidirectional=False,
                 da=100, r=10, attention_size=32, num_latent=32, dropout=0.5):
        super(NARRE_CNN_LSTM_SA, self).__init__()

        self.review_num_filters = num_filters * len(filter_sizes)
        self.review_hidden_layers = hidden_size * (1 + bidirectional)
        ...
        self.user_review_emb = ReviewCNNLSTMSA(embedding_size=word_embedding_size,
                                               filter_sizes=filter_sizes, num_filters=num_filters,
                                               hidden_size=hidden_size, num_layers=num_layers,
                                               bidirectional=bidirectional,
                                               da=da, r=r)
        self.item_review_emb = ReviewCNNLSTMSA(embedding_size=word_embedding_size,
                                               filter_sizes=filter_sizes, num_filters=num_filters,
                                               hidden_size=hidden_size, num_layers=num_layers,
                                               bidirectional=bidirectional,
                                               da=da, r=r)
        ...
        self.user_reviews_att = Attention(self.review_hidden_layers, id_embedding_size, attention_size)
        self.item_reviews_att = Attention(self.review_hidden_layers, id_embedding_size, attention_size)

        self.relu = nn.ReLU()

        self.user_latent = nn.Linear(self.review_hidden_layers, num_latent)
        self.item_latent = nn.Linear(self.review_hidden_layers, num_latent)
    ...
    def forward(self, user_id, item_id, user_reviews, item_reviews, user_rids, item_rids, rating, norm_lambda=0.001, save_att=False):
        ...
        # 2.review_embedding
        user_reviews_emb = self.user_review_emb(user_word_emb)  # # [batch, user_review_num, review_hidden_size]
        item_reviews_emb = self.item_review_emb(item_word_emb)  # [batch, user_review_num, review_hidden_layers]
        ...
        # 5.add_reviews
        user = self.dropout(torch.sum(user_reviews_emb * user_reviews_att, 1))  # [batch, review_hidden_layers]
        item = self.dropout(torch.sum(item_reviews_emb * item_reviews_att, 1))  # [batch, review_hidden_layers]
